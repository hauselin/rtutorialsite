...
install.packages(c("tidyverse", "data.table", "broom", "dtplyr", 
                   "lme4", "lmerTest", "ggbeeswarm", "cowplot", 
                   "piecewiseSEM", "compute.es", "sjstats", "sjPlot"))
```
...
library(dtplyr); library(lme4); library(lmerTest); library(ggbeeswarm); 
library(cowplot)
```
...
```{r}
mean(c(1, 2, 3))
# same as above but explicitly specifies the base package
base::mean(c(1, 2, 3)) 

lm(mpg ~ cyl, data = mtcars)
# same as above but explicitly specifies the stats package
