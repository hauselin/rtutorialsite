---
title: "Complete vs no-pooling: Fit the same model to different groups (just one simple line of code)"
description: |
...
library(roxygen2)
knitr::opts_chunk$set(echo = TRUE, cache = FALSE, comment = NA, message = FALSE, warning = FALSE, tidy = TRUE, tidy.opts = list(width.cutoff = 80), R.options = list(width = 80))
```
...
Before the invention of multi-level models (aka linear mixed effects models), how did people analyze nested or repeated-measures data? They aggregated the data, used repeated-measures ANOVA, and did other things. One interesting approach is to repeatedly fit the same model to each group separately, and this approach seems to be first mentioned in this article [@Lorch]. If you have 10 data points per subject, you have nested data (data points nested within subjects), and you could fit 10 linear regressions, one for each subject. This approach is still quite useful, and I'll demonstrate how to do it easily with `data.table` and my `summaryh()` function from my `hausekeep` package. 
...
<aside>
The correct way to fit this model is to account for the nested structure is to use linear mixed models (lme4 package): `lmer(weight ~ Time + (1 | Chick), data = dt1[Chick <= 5])`. This approach is 'partial pooling'.
</aside>
...
```{r}
library(lme4)
lmList(weight ~ Time | Chick, data = dt1[Chick %in% 1:5])
```
