...
repository_url: ./_posts/2020-04-11-causal-inference-and-lords-paradox-change-score-or-covariate/causal-inference-and-lords-paradox-change-score-or-covariate.Rmd
---
...
Get source code for this RMarkdown script [here](./_posts/2020-04-11-causal-inference-and-lords-paradox-change-score-or-covariate/causal-inference-and-lords-paradox-change-score-or-covariate.Rmd).
...
* This difference/change-score model gives the **total effect of condition**.
...
These two approaches often lead to different results, and is known as **Lord's Paradox**. Judea Pearl[@Pearl] explains this paradox very well and has "solved" this paradox by showing that both approaches are correct, but they uncover different effects or paths in the causal diagram. 
...
What I've done is specified two regression models (see comments in below). I'm telling lavaan to estimate three path coefficients for me `a`, `b`, and `c`, and then deterine other path coefficients (total, direct, indirect) based on these three coefficients.
...
lavaan's model output contains a lot of information that isn't necessary for this tutorial, so I've summarized the main path coefficients we care about below. See figure below to see the coefficients on the DAG.
...
Note that the estimated total effect `b + a * c - a` is in fact the difference in mean change between the two conditions (70). The total effect is the sum of the direct and indirect effects.

**Conclusion**: The DAG and SEM contain all the information of covariate and change-score models!

![Estimated path coefficients](./dag2.png)

## Compare results with covariate and change-score models

To show you how DAGs and SEMs are just a series of regressions, let's see how we can compute the coefficients and effects with a series of linear regressions that are just the covariate and change-score models. Only the path coefficients are shown below.
...
Paths `b` and `c` are captured by the **covariate model**. It captures the **effect of `condition` on `post` score after adjusting for `pre` score** (and also effect of `pre` on `post` after adjusting for `condition`).

```{r}
# paths b (condition) and c (pre)
paths_bc <- lm(post ~ condition + pre, dt1) # direct effect of condition
round(coef(paths_bc)[-1], 2) # see rows 2, 3, and 5 of table above
```

The **change-score model** captures the total effect. It reflects the **effect of `condition` on the change or difference score**.

```{r}
# total effect (change score): b + a * c - a  
total <- lm(change ~ condition, dt1)
